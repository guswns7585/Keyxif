...
if [ "$#" -lt 2 ]; then
  echo "Usage: scripts/release-android.sh <versionName> <versionCode> [owner/repo]" >&2
  exit 1
fi
...
python - "$ANDROID_ROOT/app/build.gradle.kts" "$VERSION_NAME" "$VERSION_CODE" <<'PY'
import re
import sys
...
(cd "$ANDROID_ROOT" && ./gradlew :app:compileReleaseKotlin)

echo "Creating release commit and tag"
git add android/app/build.gradle.kts docs/update.json
